...
public class BattleNetAuthService : IAuthService
{
        ...
        try
        {
            ...
            var thread = new Thread(() =>
            {
                try
                {
                    ...
                    // Navigate directly to the session refresh endpoint. If already logged in, it should succeed.
                    var form = new LoginForm(tcs, BaseUrl + "/oauth2/authorization/account-settings"); 
        ...
        try
        {
            ...
            var thread = new Thread(() =>
            {
                try
                {
                    ...
                    var form = new RefreshForm(tcs, BaseUrl + "/oauth2/authorization/account-settings"); 
                    Application.Run(form);
        ...
        try
        {
            ...
            var thread = new Thread(() =>
            {
                try
                {
                    ...
                    var form = new LoginForm(tcs, url); 
                    Application.Run(form);
                }
    ...
    private class LoginForm : Form
    {
        ...
        public LoginForm(TaskCompletionSource<string?> loginTcs, string targetUrl)
        {
        ...
        public LoginForm(TaskCompletionSource<(string, string)> fetchTcs, string targetUrl)
        {
        ...
        private async void LoginForm_Load(object? sender, EventArgs e)
        {
            try
            {
                ...
                if (_isRefreshing)
                {
                    ...
                    _webView.Source = new Uri(BaseUrl + "/oauth2/authorization/account-settings");
                }
                else
                {
        ...
        private async Task SaveCookiesAsync()
        {
            try
            {
                ...
                var battlenetCookies = allCookies
                    ...
                    .Select(c => new SerializableCookie { Name = c.Name, Value = c.Value, Domain = c.Domain, Path = c.Path })
                    .ToList();
        ...
        private async void WebView_NavigationCompleted(object? sender, CoreWebView2NavigationCompletedEventArgs e)
        {
            ...
            else if (_fetchResultTcs != null)
            {
                ...
                if (_isRefreshing && (url.Contains("/overview") || url.Contains("/oauth2/authorization/account-settings")))
                {
    ...
    private class RefreshForm : Form
    {
        ...
        public RefreshForm(TaskCompletionSource<bool> refreshTcs, string targetUrl)
        {
            ...
            _closeTimer.Interval = 10000; // 10 seconds
        ...
        private async void RefreshForm_Load(object? sender, EventArgs e)
        {
            try
            {
                var userDataFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "LegionDeck", "WebView2_BNet");
        ...
        private async Task SaveCookiesAsync()
        {
            try
            {
                ...
                Directory.CreateDirectory(authTokensPath);
                await File.WriteAllTextAsync(Path.Combine(authTokensPath, "battlenet_cookies.json"), json);
                Log("Battle.net cookies saved during refresh.");
            }
        ...
        private async void WebView_NavigationCompleted(object? sender, CoreWebView2NavigationCompletedEventArgs e)
        {
            ...
            // If we land on account-settings without redirection to login, session is valid.
            if (url.Contains("/oauth2/authorization/account-settings") && !url.Contains("/login/"))
            {
            ...
            // If it redirects to login, session is not valid.
            ...
            // If it's another page (e.g., /overview), it's probably authenticated.
            else
            {
                Log($"Refresh form: Unexpected navigation during refresh: {url}. Assuming success and closing.");
                await SaveCookiesAsync(); // Still save current cookies
                _refreshResultTcs.TrySetResult(true);
                this.Close();
            }
        }
